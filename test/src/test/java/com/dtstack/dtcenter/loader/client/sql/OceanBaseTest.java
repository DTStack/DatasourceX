...
public class OceanBaseTest extends BaseTest {
    // 获取数据源 client
    private static final IClient client = ClientCache.getClient(DataSourceType.OceanBase.getVal());
    ...
    public static void beforeClass() {
        ...
        client.executeSqlWithoutResultSet(source, queryDTO);
        queryDTO = SqlQueryDTO.builder().sql("drop table if exists test.demo").build();
        client.executeSqlWithoutResultSet(source, queryDTO);
        queryDTO = SqlQueryDTO.builder().sql("create table test.demo (id int COMMENT 'id', name varchar(50) COMMENT '姓名') comment 'table comment'").build();
        client.executeSqlWithoutResultSet(source, queryDTO);
    ...
    public void getTableBySchema () {
        List tableListBySchema = client.getTableListBySchema(source, SqlQueryDTO.builder().schema("test").tableNamePattern("").limit(5).build());
    ...
    public void isDatabaseExists() {
        assert  client.isDatabaseExists(source, "test");
    }
    @Test
