...
package com.dtstack.dtcenter.common.loader.yarn.client;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.dtstack.dtcenter.common.loader.common.nosql.AbsNoSqlClient;
import com.dtstack.dtcenter.common.loader.hadoop.util.SecurityUtils;
import com.dtstack.dtcenter.common.loader.yarn.util.StateUtil;
import com.dtstack.dtcenter.common.loader.yarn.util.YarnConfUtil;
import com.dtstack.dtcenter.common.loader.yarn.util.YarnRestUtil;
        ...
        return getYarnClient(source, (yarnClient -> {
            try {
                ...
                if (CollectionUtils.isEmpty(nodes)) {
                    throw new DtLoaderException("Active nodes is empty.");
                }
            } catch (Exception e) {
                throw new DtLoaderException("Active nodes is empty.", e);
            }
            return true;
        }));
    }
    ...
    public List<YarnApplicationInfoDTO> listApplication(ISourceDTO source, YarnApplicationStatus status, String taskName, String applicationId) {
        return getYarnClient(source, yarnClient -> {
            ...
            try {
                ...
                throw new DtLoaderException("listApplication " + applicationId, e);
            }
    ...
    public YarnResourceDescriptionDTO getYarnResourceDescription(ISourceDTO source) {
        return getYarnClient(source, yarnClient -> {
            try {
                ...
                throw new DtLoaderException("getYarnResourceDescription ", e);
            }
        });
    }
    ...
    public YarnResourceDTO getYarnResource(ISourceDTO source) {
        return getYarnClient(source, yarnClient -> {
            ...
            try {
                ...
                throw new DtLoaderException("getYarnResource ", e);
            }
    ...
    public <T> T getYarnClient(ISourceDTO source, Function<YarnClient, T> yarnClientConsumer) {
        ...
        return SecurityUtils.login(() -> {
            try (YarnClient yarnClient = YarnClient.createYarnClient()) {
                ...
                throw new DtLoaderException(String.format("execute method error : %s", e.getMessage()), e);
            }
