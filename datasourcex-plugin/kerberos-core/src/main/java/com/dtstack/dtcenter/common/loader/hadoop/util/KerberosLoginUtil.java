...
public class KerberosLoginUtil {
    ...
    public static UserGroupInformation loginWithUGI(Map<String, Object> confMap) {
        ...
        synchronized (DataSourceType.class) {
            ...
            // Kerberos 认证属性
            String principal = MapUtils.getString(confMap, HadoopConfTool.PRINCIPAL);
            String keytab = MapUtils.getString(confMap, HadoopConfTool.PRINCIPAL_FILE);
            String krb5Conf = MapUtils.getString(confMap, HadoopConfTool.KEY_JAVA_SECURITY_KRB5_CONF);
            // 兼容历史逻辑
            if (StringUtils.isNotEmpty(keytab) && !keytab.contains("/")) {
                keytab = MapUtils.getString(confMap, "keytabPath");
                confMap.put(HadoopConfTool.PRINCIPAL_FILE, keytab);
            }
            ...
            confMap.putIfAbsent(HadoopConfTool.RM_PRINCIPAL, principal);
