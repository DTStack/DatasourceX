...
import com.dtstack.dtcenter.common.loader.mysql5.canal.instance.CanalInstance;
...
public class MysqlTableClient extends AbsTableClient {
    ...
    public Boolean monitorBinlog(ISourceDTO source, BinlogConfigDTO binlogConfigDTO, Consumer<BinlogCallBackLogDTO> consumer) {
        if (binlogConfigDTO == null) {
            throw new DtLoaderException("binlogConfigDTO can not be null");
        }
        // 启动canal实例
        CanalInstance instance = new CanalInstance(binlogConfigDTO);
        instance.start();
