...
import com.dtstack.dtcenter.common.loader.common.utils.ColumnUtil;
import com.dtstack.dtcenter.common.loader.common.utils.CommonUtil;
import com.dtstack.dtcenter.common.loader.common.utils.DBUtil;
import com.dtstack.dtcenter.common.loader.common.utils.FileUtil;
...
public abstract class AbsRdbmsClient implements IClient {
    ...
    public Connection getCon(ISourceDTO sourceDTO) {
        ...
        try {
            Connection conn = connFactory.getConn(sourceDTO);
            log.info("-------get connection end-------");
            return conn;
        } catch (Exception e) {
    ...
    public Boolean executeBatchQuery(ISourceDTO source, SqlQueryDTO queryDTO) {
        ...
        if (StringUtils.isBlank(sql)) {
            throw new DtLoaderException("sql 不能为空");
        }
    ...
    private boolean executeProc(ISourceDTO source, SqlQueryDTO queryDTO, String procName) {
        ...
        try {
            ...
            throw new DtLoaderException(String.format("execute sql error: %s", e.getMessage()), e);
        } finally {
            closeDBResources(procStmt, null);
            ...
            closeDBResources(callStmt, conn);
        }
    }
    ...
    private void closeDBResources(Statement stmt, Connection conn) {
        try {
            ...
            log.error("close db resource error ", t);
        }
    }
    ...
    private boolean executeWithTransaction(ISourceDTO source, List<String> sqlList) {
        ...
        try {
            ...
            closeDBResources(statement, conn);
        }
    }
    ...
    public Boolean writeByFile(ISourceDTO sourceDTO, WriteFileDTO dto) {
        ...
        if (StringUtils.isBlank(dto.getTableName())) {
            throw new DtLoaderException("it must need tableName when query columns");
        }
        ...
        try (Connection con = getCon(sourceDTO);
            ...
            throw new DtLoaderException("write data by file error,detail: " + e);
        }
    ...
    private int checkWriteFileParam(WriteFileDTO dto, List<ColumnMetaDTO> dbColumns) {
        ...
        if (matchType == null) {
            throw new DtLoaderException("matchType can't be null");
        }
        if (CollectionUtils.isEmpty(dbColumns)) {
            throw new DtLoaderException("table of database doesn't have columns");
        }
        if (CollectionUtils.isEmpty(importColumns)) {
            throw new DtLoaderException("importColumns can't be null");
        }
        ...
        if (ImportDataMatchType.BY_POS.equals(matchType)) {
            ...
            if (!dto.getTopLineIsTitle()) {
                throw new DtLoaderException("topLineIsTitle param must be true when write by name");
            }
    ...
    private void parseNameByFirstLine(String separator,
        ...
        if (ImportDataMatchType.BY_NAME.equals(matchType)) {
            ...
            if (split.length < validColumns.size()) {
                throw new DtLoaderException("column size in file is not correct, columns in file: "
