...
public class CspS3Client extends AbsNoSqlClient {
    ...
    public Boolean testCon(ISourceDTO source) {
        ...
        try {
            ...
            throw new DtLoaderException(String.format("csp s3 connection failed : %s", e.getMessage()), e);
        } finally {
    ...
    public List<String> getTableList(ISourceDTO source, SqlQueryDTO queryDTO) {
        ...
        try {
            ...
            throw new DtLoaderException(String.format("csp s3 get buckets failed : %s", e.getMessage()), e);
        } finally {
            CspS3Util.closeAmazonS3(cosClient);
        }
