    ...
    public Boolean testCon(ISourceDTO source) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(kubernetesSourceDTO.getNamespace())
                ...
                throw new DtLoaderException(String.format("namespace [%s] not found", kubernetesSourceDTO.getNamespace()), e);
            } else if (e.getMessage().contains("not match") || e.getMessage().contains("doesn't have permission")) {
                throw new DtLoaderException(String.format("namespace [%s] not found or doesn't hive permission", kubernetesSourceDTO.getNamespace()), e);
            } else {
                throw new DtLoaderException(String.format("test kubernetes connection error: %s", e.getMessage()), e);
            }
        } finally {
