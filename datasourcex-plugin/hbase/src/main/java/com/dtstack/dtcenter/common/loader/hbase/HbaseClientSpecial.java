...
public class HbaseClientSpecial implements IHbase {
    ...
    @Override
    public Boolean createHbaseTable(ISourceDTO source, String namespace, String tbName, Map<String, String> colFamily) {
        colFamily.keySet().removeIf(Objects::isNull);
        if (MapUtils.isEmpty(colFamily)){
            throw new DtLoaderException("Table should have at least one column family");
        }
        return createTable(source, namespace, tbName, colFamily);
    }

    @Override
    public Boolean createHbaseTable(ISourceDTO source, String namespace, String tbName, String[] colFamily) {
        if (ArrayUtils.isEmpty(colFamily)){
            throw new DtLoaderException("Table should have at least one column family");
        }
        Map<String, String> colMappings = new HashMap<>();
        Arrays.stream(colFamily).forEach(colName -> colMappings.put(colName, "none"));
        return createTable(source, namespace, tbName, colMappings);
    }

    private Boolean createTable(ISourceDTO source, String namespace, String tbName, Map<String, String> colFamily) {
        ...
        try {
            ...
            if (admin.tableExists(tableName)) {
                ...
                for (Map.Entry<String, String> entry : colFamily.entrySet()) {
                    ...
                    if (StringUtils.isNotBlank(entry.getValue())) {
                        hColumnDescriptor.setCompressionType(AlgorithmType.getType(entry.getValue()));
                    }
            ...
            throw new DtLoaderException(String.format(
                    "hbase failed to create table！namespace：'%s'，table name：'%s'，column family：'%s'",
                    namespace, tbName, colFamily.keySet()), e);
        } finally {
            close(admin);
            closeConnection(connection, hbaseSourceDTO);
            HbaseClient.destroyProperty();
    ...
    public List<Map<String, Object>> executeQuery(ISourceDTO source, HbaseQueryDTO hbaseQueryDTO, TimestampFilter timestampFilter) {
        ...
        try {
            ...
            table = RetryUtil.executeWithRetry(() -> finalConnection.getTable(tableName), DEFAULT_RETRY_NUM, DEFAULT_RETRY_INTERVAL, false);
            Scan scan = new Scan();
