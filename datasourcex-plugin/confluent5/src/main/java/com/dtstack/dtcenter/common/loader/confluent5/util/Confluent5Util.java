...
import com.dtstack.dtcenter.common.loader.confluent5.Confluent5Consistent;
import com.dtstack.dtcenter.common.loader.confluent5.enums.EConsumeType;
...
public class Confluent5Util {
    ...
    private static String writeKafkaJaas(Map<String, Object> kerberosConfig) {
        ...
        try {
            ...
            FileUtils.write(jaas, String.format(Confluent5Consistent.KAFKA_JAAS_CONTENT, keytabConf, principal));
    ...
    public static String getAllBrokersAddressFromZk(String zkUrls) {
        ...
        try {
            zkUtils = ZkUtils.apply(zkUrls, Confluent5Consistent.SESSION_TIME_OUT,
                    Confluent5Consistent.CONNECTION_TIME_OUT, JaasUtils.isZkSecurityEnabled());
     ...
     *
     * @param confluent5SourceDTO kafka 数据源信息
     * @return topic 列表
     */
    public static List<String> getTopicList(Confluent5SourceDTO confluent5SourceDTO) {
        Properties defaultKafkaConfig = initProperties(confluent5SourceDTO);
     ...
     */
    public static void createTopicFromBroker(Confluent5SourceDTO sourceDTO, String topicName,
     ...
     */
    public static List<KafkaOffsetDTO> getPartitionOffset(Confluent5SourceDTO sourceDTO, String topic) {
        Properties defaultKafkaConfig = initProperties(sourceDTO);
        try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(defaultKafkaConfig)) {
    ...
    /**
     * 根据 Kafka 地址校验连接性、校验 schema Registry 地址是否正确
     *
     * @param sourceDTO 数据源信息
     * @return 是否连通
     */
    public static boolean checkConnection(Confluent5SourceDTO sourceDTO) {
        // 校验 schema registry 地址是否正确
        checkSchemaRegistry(sourceDTO.getSchemaRegistryUrl());
    ...
    /**
     * 校验 SchemaRegistry 地址连通性
     * @param schemaRegistryUrl schema 注册中心地址
     */
    private static void checkSchemaRegistry(String schemaRegistryUrl) {
        if (StringUtils.isEmpty(schemaRegistryUrl)) {
            return;
        }
        CachedSchemaRegistryClient client = new CachedSchemaRegistryClient(schemaRegistryUrl, 10);
        try {
            client.getAllSubjects();
        } catch (Exception e) {
            throw new DtLoaderException(String.format("check schema registry url error: %s", e.getMessage()), e);
        }
    }
     ...
     * @return kafka broker 地址
     */
    private static String getKafkaBroker(Confluent5SourceDTO sourceDTO) {
     ...
     * @return kafka 配置
     */
    private synchronized static Properties initProperties(Confluent5SourceDTO sourceDTO) {
        try {
            ...
            /* 设置group id */
            props.put("group.id", Confluent5Consistent.KAFKA_GROUP);
            /* 自动确认offset的时间间隔 */
            ...
            if (StringUtils.isNotEmpty(sourceDTO.getSchemaRegistryUrl())) {
                props.put("schema.registry.url", sourceDTO.getSchemaRegistryUrl());
            }
            ...
            if (StringUtils.isNotBlank(sourceDTO.getUsername()) && StringUtils.isNotBlank(sourceDTO.getPassword())) {
                ...
                props.put("sasl.jaas.config", String.format(Confluent5Consistent.KAFKA_SASL_PLAIN_CONTENT, sourceDTO.getUsername(), sourceDTO.getPassword()));
                return props;
            }

            SSLUtil.SSLConfiguration sslConfiguration = SSLUtil.getKafkaSSLConfiguration(sourceDTO);
            // sslConfigDTO 不为空标识开启 ssl 认证
            if (Objects.nonNull(sslConfiguration)) {
                PropertiesUtil.setIgnoreEmpty(props, "security.protocol", "SSL");
                PropertiesUtil.setIgnoreEmpty(props, "ssl.truststore.location", sslConfiguration.getTrustStorePath());
                PropertiesUtil.setIgnoreEmpty(props, "ssl.truststore.password", sslConfiguration.getTrustStorePassword());
                PropertiesUtil.setIgnoreEmpty(props, "ssl.keystore.location", sslConfiguration.getKeyStorePath());
                PropertiesUtil.setIgnoreEmpty(props, "ssl.keystore.password", sslConfiguration.getKeyStorePassword());
    ...
    public static List<String> getRecordsFromKafka(Confluent5SourceDTO sourceDTO, String topic, String autoReset) {
    ...
    public static List<KafkaPartitionDTO> getPartitions (Confluent5SourceDTO sourceDTO, String topic) {
        Properties defaultKafkaConfig = initProperties(sourceDTO);
     ...
     * @return 消费到的数据
     */
    public static List<String> consumeData(Confluent5SourceDTO sourceDTO, String topic, Integer collectNum,
     ...
     * @return 消费者组列表
     */
    public static List<String> listConsumerGroup(Confluent5SourceDTO sourceDTO, String topic) {
     ...
     * @return 消费者组详细信息
     */
    public static List<KafkaConsumerDTO> getGroupInfoByGroupId(Confluent5SourceDTO sourceDTO, String groupId, String srcTopic) {
