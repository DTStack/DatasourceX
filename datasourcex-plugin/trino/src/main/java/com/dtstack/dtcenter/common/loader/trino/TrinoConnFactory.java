...
public class TrinoConnFactory extends ConnFactory {
    ...
    private Connection getConnAndSetSchema(String url, String catalog, String schema, Properties properties) throws SQLException {
        ...
        checkCatalogSchemaExists(realCatalog, realSchema, connection);
        return connection;
    }
     ...
     */
    private void checkCatalogSchemaExists(String catalog, String schema, Connection connection) {
        // catalog 不存在直接跳过校验
        ...
        try (ResultSet catalogs = connection.getMetaData().getCatalogs()){
            ...
            if (checkAccessDenied(e)) {
                ...
                throw new DtLoaderException(String.format("check catalog [%s] exists error: %s", catalog, e.getMessage()), e);
            }
        }
        if (!catalogExists) {
            throw new DtLoaderException(String.format("catalog %s does not exist.", catalog));
        }
        ...
        try (ResultSet schemas = connection.getMetaData().getSchemas(catalog, null)){
            ...
            if (checkAccessDenied(e)) {
                ...
                throw new DtLoaderException(String.format("check schema [%s] exists error: %s", schema, e.getMessage()), e);
            }
        }
        if (!schemaExists) {
            throw new DtLoaderException(String.format("schema %s does not exist.", schema));
        }
    }
