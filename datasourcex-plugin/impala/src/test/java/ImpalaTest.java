...
public class ImpalaTest {
    ...
    public void getFlinkColumnMetaData() throws Exception {
        ImpalaSourceDTO source = ImpalaSourceDTO.builder()
                .url("jdbc:impala://172.16.100.169:21050/xingran")
                .defaultFS("hdfs://ns1")
                .schema("xingran")
                .build();
        IClient client = new ImpalaClient();
        List<ColumnMetaDTO> columnMetaDTOS = client.getFlinkColumnMetaData(source, SqlQueryDTO.builder().tableName("baixi_side").build());
        AssertUtils.notNull(columnMetaDTOS, "");
    }
}
