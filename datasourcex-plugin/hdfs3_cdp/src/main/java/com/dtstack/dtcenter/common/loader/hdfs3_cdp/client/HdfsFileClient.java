...
package com.dtstack.dtcenter.common.loader.hdfs3_cdp.client;
...
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.HdfsCsvDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.HdfsFileDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.HdfsORCDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.HdfsParquetDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.HdfsTextDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.YarnLogDownload.YarnIFileDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.downloader.YarnLogDownload.YarnTFileDownload;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.fileMerge.core.CombineMergeBuilder;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.fileMerge.core.CombineServer;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.hdfswriter.HdfsOrcWriter;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.hdfswriter.HdfsParquetWriter;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.hdfswriter.HdfsTextWriter;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.reader.ReaderFactory;
import com.dtstack.dtcenter.common.loader.hdfs3_cdp.util.StringUtil;
...
import com.dtstack.dtcenter.loader.dto.source.Hdfs3SourceDTO;
...
public class HdfsFileClient implements IHdfsFile {
    ...
    public List<String> getTaskManagerList(ISourceDTO source, String submitUser, String appId) {
        Hdfs3SourceDTO hdfs3SourceDTO = (Hdfs3SourceDTO) source;
        hdfs3SourceDTO.setUser(submitUser);
        hdfs3SourceDTO.setAppIdStr(appId);
        return KerberosLoginUtil.loginWithUGI(hdfs3SourceDTO.getKerberosConfig()).doAs(
                (PrivilegedAction<List<String>>) () -> {
                    AbstractYarnDownloader yarnLogDownload = null;
                    try {
                        yarnLogDownload = createYarnLogDownload(hdfs3SourceDTO);
     ...
     */
    private AbstractYarnDownloader createYarnLogDownload(Hdfs3SourceDTO hdfsSourceDTO) throws Exception {
        AbstractYarnDownloader yarnDownload;
        ...
        if (StringUtils.isNotBlank(fileFormat) && StringUtils.containsIgnoreCase(fileFormat, "IFile")) {
            yarnDownload = new YarnIFileDownload(hdfsSourceDTO.getDefaultFS(), hdfsSourceDTO.getUser(), hdfsSourceDTO.getConfig(), hdfsSourceDTO.getYarnConf(), hdfsSourceDTO.getAppIdStr(), hdfsSourceDTO.getReadLimit(), hdfsSourceDTO.getLogType(), hdfsSourceDTO.getKerberosConfig(), hdfsSourceDTO.getContainerId());
        } else {
    ...
    public IDownloader getDownloaderByFormatWithType(ISourceDTO source, String tableLocation, List<ColumnMetaDTO> allColumns, List<String> filterColumns, Map<String, String> filterPartition, List<String> partitions, String fieldDelimiter, String fileFormat) {
        return getDownloaderByFormatWithType(source, tableLocation, allColumns, filterColumns, filterPartition, partitions, fieldDelimiter, fileFormat, false);
    }
    ...
    public IDownloader getDownloaderByFormatWithType(ISourceDTO source, String tableLocation, List<ColumnMetaDTO> allColumns, List<String> filterColumns, Map<String, String> filterPartition, List<String> partitions, String fieldDelimiter, String fileFormat, Boolean isTransTable) {
        ...
        return KerberosLoginUtil.loginWithUGI(hdfsSourceDTO.getKerberosConfig()).doAs(
                (PrivilegedAction<IDownloader>) () -> {
                    try {
                        Configuration conf = HadoopConfUtil.getHdfsConf(hdfsSourceDTO.getDefaultFS(), hdfsSourceDTO.getConfig(), hdfsSourceDTO.getKerberosConfig());
                        if (BooleanUtils.isTrue(isTransTable)) {
    ...
    public void initTransConfig(Configuration conf, ArrayList<ColumnMetaDTO> columns) {
        ...
        conf.set(IOConstants.SCHEMA_EVOLUTION_COLUMNS, jodd.util.StringUtil.join(columnNames, ","));
        conf.set(IOConstants.SCHEMA_EVOLUTION_COLUMNS_TYPES, jodd.util.StringUtil.join(columnTypes, ","));
