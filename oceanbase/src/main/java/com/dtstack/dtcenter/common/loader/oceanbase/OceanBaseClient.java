 ...
 */
@Slf4j
public class OceanBaseClient<T> extends AbsRdbmsClient<T> {
    // 获取正在使用数据库
    ...
    public String getTableMetaComment(ISourceDTO iSource, SqlQueryDTO queryDTO) {
        OceanBaseSourceDTO oceanBaseSourceDTO = (OceanBaseSourceDTO) iSource;
        Integer clearStatus = beforeColumnQuery(oceanBaseSourceDTO, queryDTO);
        Statement statement = null;
        ResultSet resultSet = null;

        try {
            statement = oceanBaseSourceDTO.getConnection().createStatement();
