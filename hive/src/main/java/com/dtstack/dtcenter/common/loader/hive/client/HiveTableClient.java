 ...
 */
@Slf4j
public class HiveTableClient implements ITable {

    private static final IClient hiveClient = ClientCache.getClient(DataSourceType.HIVE.getVal());
    ...
    public List<String> showPartitions(ISourceDTO source, String tableName) throws Exception {
        log.info("hive2.x获取表分区，表名：{}", tableName);
        if (StringUtils.isBlank(tableName)) {
            throw new DtLoaderException("查询表不能为空！");
        }
        List<Map<String, Object>> result = hiveClient.executeQuery(source, SqlQueryDTO.builder().sql(String.format(SHOW_PARTITIONS_SQL, tableName)).build());
    ...
    public Boolean renameTable(ISourceDTO source, String oldTableName, String newTableName) throws Exception {
        ...
        return hiveClient.executeSqlWithoutResultSet(source, SqlQueryDTO.builder().sql(renameTableSql).build());
    }

    @Override
    public Boolean alterTableParams(ISourceDTO source, String tableName, Map<String, String> params) throws Exception {
        log.info("hive2.x更改表参数，表名：{}，参数：{}", tableName, params);
        if (StringUtils.isBlank(tableName)) {
            throw new DtLoaderException("表名不能为空！");
        }
        ...
        return hiveClient.executeSqlWithoutResultSet(source, SqlQueryDTO.builder().sql(alterTableParamsSql).build());
    }
}
