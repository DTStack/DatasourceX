...
public class RocketMqClientFactory {
    public static IRocketMq createPluginClass(String pluginName) throws Exception {
        ClassLoader classLoader = ClassLoaderCache.getClassLoader(pluginName);
        return ClassLoaderCallBackMethod.callbackAndReset((ClassLoaderCallBack<IRocketMq>) () -> {
            ServiceLoader<IRocketMq> rocketMqs = ServiceLoader.load(IRocketMq.class);
            Iterator<IRocketMq> iClientIterator = rocketMqs.iterator();
            if (!iClientIterator.hasNext()) {
                throw new DtLoaderException("This plugin type is not supported: " + pluginName);
            }

            IRocketMq rocketMq = iClientIterator.next();
            return new RocketMqProxy(rocketMq);
        }, classLoader);
    }
}
