...
public class EsClient<T> extends AbsNoSqlClient<T> {
    ...
    private static RestHighLevelClient getClient(ESSourceDTO esSourceDTO) {
        String keyPath = ReflectUtil.fieldExists(ESSourceDTO.class, "keyPath") ? esSourceDTO.getKeyPath() : null;
        //开启 ssl 不开连接池
        boolean check = esSourceDTO.getPoolConfig() != null && keyPath == null;
        IS_OPEN_POOL.set(check);
        if (!check) {
            return getClient(esSourceDTO.getUrl(), esSourceDTO.getUsername(), esSourceDTO.getPassword(), keyPath);
        }
