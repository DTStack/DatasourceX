...
package com.dtstack.dtcenter.loader.engine;

import com.alibaba.fastjson.JSONObject;
import com.dtstack.dtcenter.loader.dto.SSLConfig;
...
import com.dtstack.dtcenter.loader.dto.source.Huawei3YarnSourceDTO;
...
import com.dtstack.dtcenter.loader.dto.source.NfsSourceDTO;
...
import com.dtstack.dtcenter.loader.dto.source.SapHana2SourceDTO;
...
import com.dtstack.dtcenter.loader.dto.source.TBDSYarn2SourceDTO;
import com.dtstack.dtcenter.loader.dto.source.TbdsHdfsSourceDTO;
import com.dtstack.dtcenter.loader.dto.source.TiDBSourceDTO;
import com.dtstack.dtcenter.loader.dto.source.TrinoSourceDTO;
import com.dtstack.dtcenter.loader.dto.source.Yarn2SourceDTO;
import com.dtstack.dtcenter.loader.dto.source.Yarn3SourceDTO;
import com.dtstack.dtcenter.loader.dto.source.YarnSourceDTO;
import com.dtstack.dtcenter.loader.exception.DtLoaderException;
import com.dtstack.dtcenter.loader.source.DataSourceType;
import com.dtstack.dtcenter.loader.utils.AssertUtils;
import com.dtstack.dtcenter.loader.utils.MapUtils;
...
public enum PluginInfoToSourceDTO {
     ...
     * HUAWEI_HDFS
     */
    HUAWEI_HDFS(DataSourceType.HUAWEI_HDFS.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return HuaweiHdfsSourceDTO.builder()
                    .defaultFS(getDefaultFS(pluginInfo))
                    .config(getHadoopConfig(pluginInfo)).build();
        }
    },
     ...
     * yarn2tbds
     */
    TBDS_YARN2(DataSourceType.TBDS_YARN2.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return fillYarnSourceDTO(TBDSYarn2SourceDTO.builder().build(), pluginInfo);
        }
    },
     ...
     * yarn3hw
     */
    HUAWEI_YARN3(DataSourceType.HUAWEI_YARN3.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return fillYarnSourceDTO(Huawei3YarnSourceDTO.builder().build(), pluginInfo);
        }
    },
     ...
     * TBDS_HDFS
     */
    TBDS_HDFS(DataSourceType.TBDS_HDFS.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return TbdsHdfsSourceDTO.builder()
                    .defaultFS(getDefaultFS(pluginInfo))
                    .config(getHadoopConfig(pluginInfo)).build();
        }
    },
     ...
     * aws S3
     */
    AWS_S3(DataSourceType.CSP_S3.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
     ...
     * RANGER
     */
    RANGER(DataSourceType.RANGER.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return RangerSourceDTO
                    .builder()
     ...
     * LDAP
     */
    LDAP(DataSourceType.LDAP.getVal()) {
        @Override
        public ISourceDTO getSourceDTO(JSONObject pluginInfo) {
            return LdapSourceDTO
                    .builder()
                    .url(pluginInfo.getString(SourceConstant.URL))
                    .username(pluginInfo.getString(SourceConstant.USERNAME))
                    .password(pluginInfo.getString(SourceConstant.PASSWORD))
    ...
    public static ISourceDTO getSourceDTO(String data, Long dtUicTenantId) {
        ...
        fillOtherConfig(sourceDTO, pluginInfo, dtUicTenantId);
        return sourceDTO;
    }
     ...
     */
    private static void fillOtherConfig(ISourceDTO sourceDTO, JSONObject pluginInfo, Long dtUicTenantId) {
        AbstractSourceDTO abstractSourceDTO = (AbstractSourceDTO) sourceDTO;
        abstractSourceDTO.setTenantId(dtUicTenantId);
        abstractSourceDTO.setSftpConf(MapUtils.toStringMap(pluginInfo.getJSONObject(SourceConstant.SFTP_CONF)));
        // 开启 kerberos
        if (Objects.equals(1, pluginInfo.getInteger(SourceConstant.OPEN_KERBEROS))) {
            ...
            kerberosConf.put(SourceConstant.SFTP_CONF, MapUtils.toStringMap(pluginInfo.getJSONObject(SourceConstant.SFTP_CONF)));
    ...
    private static JSONObject getDataSourceJson(String pluginInfoStr) {
        if (StringUtils.isBlank(pluginInfoStr)) {
            throw new DtLoaderException("数据源信息为空");
        }
        try {
            return JSONObject.parseObject(pluginInfoStr);
        } catch (Exception e) {
            throw new DtLoaderException(String.format("转化json格式异常：%s", e.getMessage()), e);
        }
    }
    ...
    private static PluginInfoToSourceDTO getSourceDTOType(Integer sourceType) {
        ...
        throw new DtLoaderException(String.format("找不到对应的数据源类型, sourceType：%s", sourceType));
    }
